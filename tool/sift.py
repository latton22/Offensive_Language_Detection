...
##### LUKE用の変更点
# 1. LayerNormの初期化時のパラメータが異なる.
# self.LayerNorm = torch.nn.LayerNorm(hidden_size, 1e-7, elementwise_affine=False)
#   --> self.LayerNorm = torch.nn.LayerNorm(hidden_size, 1e-5, elementwise_affine=False) # 1e-5がLUKEで使用されている値.
# 2. LUKEにはentityのエンべディング層(entity_embedding.LayerNorm)もあって, デフォルトの指定方法だとこれも選択されてエラーになる.
# target_module = 'embeddings.LayerNorm'
#   --> target_module = '.embeddings.LayerNorm' # . を追加して, 選択範囲を限定
...
class PerturbationLayer(torch.nn.Module):
	def __init__(self, hidden_size, learning_rate=1e-4, init_perturbation=1e-2):
		...
		self.delta = None
		self.LayerNorm = torch.nn.LayerNorm(hidden_size, 1e-5, elementwise_affine=False) # DeBERTa: 1e-7 --> LUKE: 1e-5
